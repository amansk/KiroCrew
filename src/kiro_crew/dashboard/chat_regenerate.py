...
from kiro_crew.dashboard.kiro_readiness import live_session_backend, reject_if_kiro_unverified
...
async def api_chat_slot_regenerate(request: web.Request) -> web.Response:
    ...
    blocked = await reject_if_kiro_unverified(
        request, backend=live_session_backend(state, effective_session_key(slot))
    )
