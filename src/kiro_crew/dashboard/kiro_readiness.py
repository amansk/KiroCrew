...
(:func:`selected_backend_signs_in_via_kiro_cli`). On a deployment whose
...
async def selected_backend_signs_in_via_kiro_cli() -> bool:
    """:func:`backend_signs_in_via_kiro_cli` over :func:`selected_backend`."""

    return backend_signs_in_via_kiro_cli(await selected_backend())


def live_session_backend(state: object, session_key: str) -> str | None:
    """The backend a LIVE session on *session_key* runs on, or ``None`` if none.
    ...
    Peeks only, never creates: ``state.sessions._sessions`` is read the way
    ``chat_runner`` reads it for ``/compact``. Anything that is not a live
    session carrying a string backend answers ``None`` -- a mocked manager, a
    non-ACP provider, a key with no session -- and the caller then falls back to
    the configured backend, which is what a fresh session would get.
    """

    sessions = getattr(getattr(state, "sessions", None), "_sessions", None)
    if not isinstance(sessions, dict):
        return None
    backend = getattr(getattr(sessions.get(session_key), "provider", None), "acp_backend", None)
    return backend if isinstance(backend, str) else None
...
async def reject_if_kiro_unverified(
    ...
    * **``POST /v1/chat/completions``** — no transcript the caller reads. Its
    ...
    session passes that session's backend (:func:`live_session_backend`), which
    survives a hot switch of the default. Left ``None``, the configured backend
    ...
    if backend_signs_in_via_kiro_cli(backend):
