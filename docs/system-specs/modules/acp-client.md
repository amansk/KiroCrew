...
- **Poll-driven spawn sites are readiness-gated.** `kiro-cli` auto-launches an
  ...
  - **The latch governs only a harness that signs in through kiro-cli.** The
    ...
    by `kiro_readiness.selected_backend_signs_in_via_kiro_cli()`. For any other
  ...
  - **A caller that continues a live session gates on THAT session's backend.**
    ...
    the rerun still lands on the old kiro-cli. `kiro_readiness.live_session_backend`
    peeks `state.sessions._sessions` (never creates) and reads
    `LLMProvider.acp_backend` — declared on the ABC with a `None` default (H14),
    answered by `AcpProvider` from its client; `None` falls back to the configured
    backend, which is what a fresh session would get. Edit-resend and rewind
    discard the conversation first, so the configured backend is theirs.
