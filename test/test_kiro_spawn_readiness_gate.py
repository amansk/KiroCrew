...
def test_live_session_backend_peeks_a_live_session_and_ignores_everything_else() -> None:
    live = SimpleNamespace(
        sessions=SimpleNamespace(
            _sessions={"k": SimpleNamespace(provider=SimpleNamespace(acp_backend=ACP_BACKEND_KIRO))}
        )
    )
    assert kiro_readiness.live_session_backend(live, "k") == ACP_BACKEND_KIRO
    # No session on that key: the caller falls back to the configured backend.
    assert kiro_readiness.live_session_backend(live, "other") is None
    # A mocked manager (every attribute truthy) must not read as a backend.
    assert kiro_readiness.live_session_backend(SimpleNamespace(sessions=MagicMock()), "k") is None
    # A provider that is not an ACP harness answers None on the ABC default.
    non_acp = SimpleNamespace(
        sessions=SimpleNamespace(_sessions={"k": SimpleNamespace(provider=SimpleNamespace())})
    )
    assert kiro_readiness.live_session_backend(non_acp, "k") is None
